...
phi_label = tk.Label(window, text="Select the throwing angle of the projectile")
phi_scale = tk.Scale(window, from_=0, to=90, length=600, tickinterval=5, orient=tk.HORIZONTAL)
...
def adjust():
    if f_plot:
        ...
        phi = get_phi()
        g = 9.81
        t_scale.set(float(calculates.quad(-g/2,v*math.sin(np.deg2rad(phi)),h)))
        print("t")
        print(get_t())
        print("--")
        do_calculations()
